...
<body>
     ...
     <center>
      ...
      <div>
         <a href="list?pageNo=1">首页</a>
         <c:choose>
             <c:when test="${pageNo>1}">
                <a href="list?pageNo=${pageNo-1}">上一页</a>
             </c:when>
             <c:otherwise>
                 <a href="list?pageNo=1">上一页</a>
             </c:otherwise>
         </c:choose>
          ...
          <c:forEach  begin="1"  end="${totalPage}"  var="n">
                <c:choose>
                      <c:when test="${n==pageNo}">
                         <a href="list?pageNo=${n}"  class='on'>${n}</a>
                      </c:when>
                     <c:otherwise>
                        <a href="list?pageNo=${n}" >${n}</a>
                     </c:otherwise>
                </c:choose>
          ...
          <c:choose>
             <c:when test="${pageNo<totalPage}">
                 <a href="list?pageNo=${pageNo+1}">下一页</a>
             </c:when>
            <c:otherwise>
              <a href="list?pageNo=${totalPage}">下一页</a>
            </c:otherwise>
          </c:choose>
           
          <a href="list?pageNo=${totalPage}">尾页</a>
      </div>
    <div>    
     <a href="toAdd">添加</a>
     </div>
